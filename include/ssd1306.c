...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  {
    return; // Se for um caractere não suportado, retorna sem desenhar
  }

  // Se for uma letra minúscula, aplicamos a rotação correta
  if (c >= 'a' && c <= 'z')
  {
    for (uint8_t i = 0; i < 8; ++i)
    {
      uint8_t linha_original = font[index + i];
      for (uint8_t j = 0; j < 8; ++j)
      {
        // Corrigimos a rotação para que fique corretamente orientada
        ssd1306_pixel(ssd, x + (7 - j), y + i, (linha_original & (1 << j)) ? 1 : 0);
      }
    }
  }
  else
  {
    // Para caracteres normais (números e maiúsculas), desenha sem rotação
    for (uint8_t i = 0; i < 8; ++i)
    {
...
//     // Aplicar transposição de bits SOMENTE nas letras minúsculas
//     if (c >= 'a' && c <= 'z')
//     {
//         for (uint8_t j = 0; j < 8; j++)
//         {
...
//     else
//     {
//         novo = line; // Mantém o original para outros caracteres
//     }
